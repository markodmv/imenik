...
<title>Ažuriranje izostanaka</title>
<style type="text/css">
<!--
body,td,th {
...
<table width="300" height="506" border="0" cellpadding="0" cellspacing="0">
 ...
 <ul>
  <ul>
	...
	<ul>
  <ul>
    <li><a href="izostanci.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>">
    Povratak</a></li> 
	</ul>                                     
  </ul>
	<hr width="180">
 ...
 <table width="700" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  echo "Izostanci :<br><br>";
   	  if (!$_POST["SBunos"])
	{
	echo "<br>Odaberite samo jedan datum za koji želite ažurirati izostanke!<br>";
  ?>
  ...
  <table width="600" border="1" cellspacing="0" cellpadding="0">
    <tr bgcolor="#BEB4FC">
      ...
      <td width="30" ><div align="center" class="style16">
        <div align="center">O</div>
      </div></td>
      <td ><div align="center" class="style16">N</div></td>
    </tr>
	<?	
	$sql="SELECT * from izostanci where opu_id = '$opu' order by datum desc";
	...
	if (mysql_num_rows($q)==0)
		{
				...
				while ($redak=mysql_fetch_array($q))
					...
					$azur="select izostanci_id, razlog_izostanka from izostanci_azuriranje where izostanci_id = '$iz'";
					if(!$az=mysql_query($azur)) {echo "<br>Nastala je greška pri izvođenju upita!<br>".mysql_query(); die();}
					if (mysql_num_rows($az)==1) 
						{//echo "<br>Svi izostanci su ažurirani!<br><br><br>";
						} 
					else
					{
	?>
	<tr>
      <td width="80"><div align="center" class="style14"><input type="hidden" name="dat_izo" value="<?=$redak["datum"]?>" /><?=$redak["datum"]?></div></td>
      <td width="140"><div align="center" class="style14">
	  <?=$redak["prvi"]?>
	  <?=$redak["drugi"]?>
	  <?=$redak["treci"]?>
	  <?=$redak["cetvrti"]?>
      ...
      <td width="310"><div align="center" class="style14"><input type="checkbox" name="izo[]" value="<?=$iz?>" /><input type="text" name="razlog_izostanka<?=$iz?>"  ></div></td>
      <td width="30"><div align="center" class="style16"><input name="opravdano<?=$iz?>" type="text" size="1" /></div></td>
      <td width="30"><div align="center" class="style16"><input name="neopravdano<?=$iz?>" type="text" size="1" /></div></td>
    </tr>
	<?
					}
					}
  ...
  <?
		...
		{
		if (is_array($_POST["izo"]))
{	
		foreach($_POST["izo"] as $izos)
		{
		$izostanak="Select * from izostanci where izostanci_id = '$izos'";
	  if(!$izostan=mysql_query($izostanak)) {echo "Nastala je greška pri izvođenju upita!<br />" . mysql_query(); }
	  while ($redak=mysql_fetch_array($izostan))
	  {
	  	$iz= $redak["izostanci_id"];
		$o= $_POST["opravdano". $iz];
		$n= $_POST["neopravdano". $iz];
		$ri= $_POST["razlog_izostanka". $iz];
		if($ri > null)
			{
			if($o > 0 or $n > 0)
				{
		$dat= date("Y")."-".date("m")."-".date("d");
		$izosta="insert into izostanci_azuriranje(korisnik_id, razred_id, opu_id, izostanci_id, razlog_izostanka, opravdano, neopravdano, datum, datum_izostanka) values ('$kid', '$rid', '$opu', '$izos', '$ri', '$o', '$n', '$dat', '$_POST[dat_izo]')";
		if (mysql_query($izosta))
					{
				header("Location: izostanci.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]");
					}
					else
						{
						echo "<br>Podaci nisu pohranjeni!<br />" .mysql_error();
						?>
	<a href="izostanci_azuriranje.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
						}
				}
				else
					{
					echo "<br>Upiši broj opravdanih ili neopravdanih sati!<br>";
					?>
	<a href="izostanci_azuriranje.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
					}		
				...
				{
				echo "<br>Upiši razlog izostanka!<br>";
				?>
	<a href="izostanci_azuriranje.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
				}		
	...
	{
	echo "<br>Odaberite samo jedan datum za izostanke koje želite ažurirati!<br>";
	?>
	<a href="izostanci_azuriranje.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
	}		

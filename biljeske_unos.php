...
<table width="300" height="506" border="0" cellpadding="0" cellspacing="0">
 ...
 <ul>
  ...
  <hr width="180">
	<ul>
  <ul>
    <li><a href="biljeske.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>">
    Povratak</a></li>
	</ul>                                     
  </ul>
	<hr width="180">
 ...
 <br />	 
  ...
  if (!$_POST["SBunos"])
	...
	<?
		...
		{
		if ($_POST["tekst"] == null){header("Location: biljeske.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]");}
		else
			{
		$dat= date("Y")."-".date("m")."-".date("d");
		$bilj="insert into biljeske(biljeska, razred_id, korisnik_id, opu_id, datum) values ('$_POST[tekst]', '$rid', '$kid', '$opu', '$dat')";
		if(!mysql_query($bilj))
				{
				echo "<br>Došlo je do greške pri unosu!<br>". mysql_error();
				?>
	<a href="biljeske_unos.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
				}
				else
					{ 
					header("Location: biljeske.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]");
...
if (!$_POST["Unoscl"])
	{
	  ...
	  $clanak="select cl58 from clanak58 where opu_id = '$opu'";
	  if(!$cl=mysql_query($clanak)){echo "<br>Nastala je greška prilikom izvođenja upita!<br>".mysql_query();}
    ...
    <br />
    <br />
    Učenik napreduje po čl. 58 Zakona o osnovnom školstvu
<input type="checkbox" name="cl58[]" value="Učenik napreduje po čl. 58 Zakona o osnovnom školstvu" />
    <br />
    <br />
     <input type="submit" name="Unoscl" value="Potvrdi unos">
      </form>
	    <? 
		  }
	}
	...
	{
		if(is_array($_POST["cl58"]))
		{
			foreach($_POST["cl58"] as $cl)
			{
			$dat= date("Y")."-".date("m")."-".date("d");
		$bilje="insert into clanak58(cl58, razred_id, korisnik_id, opu_id, datum) values ('$cl', '$rid', '$kid', '$opu', '$dat')";
		if(!mysql_query($bilje))
				{
				echo "<br>Došlo je do greške pri unosu!<br>". mysql_error();
				?>
	<a href="biljeske_unos.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
			...
			{
				echo "<br>Označite da želite unijeti podatak o članku 58.!<br>";
						?>
	<a href="biljeske_unos.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br />Ponovi unos!</a>
	<?
				}

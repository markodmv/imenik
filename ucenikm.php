...
if (!$q=mysql_query($sql))
	...
	if (mysql_num_rows($q)==0)
		...
		echo "Nema niti jednog učenika u imeniku!<br>";
				...
				while ($redak=mysql_fetch_array($q))
					...
					<tr>
					<td width="180">
					<a href="ucenikm.php?razred_id=<?=$redak["razred_id"]?>&opu_id=<?=$redak["opu_id"]?>&mjesto_skole=<?=$_GET["mjesto_skole"]?>&naziv_skole=<?=$_GET["naziv_skole"]?>" class="style12"> <?=$redak["rbr_ucenika"]?>. <?=$redak["prezime_ucenika"]?> <?=$redak["ime_ucenika"]?> <br /></a>					</td>
					</tr>
					<?
					}
					?>
 ...
 <table width="700" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  <?
  $opu="Select * from osobni_podaci_ucenika where opu_id = '$opu' and razred_id = '$rid'";

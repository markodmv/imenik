...
<title>Unos završnih ocjena</title>
<style type="text/css">
<!--
body,td,th {
...
<table width="700" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<?
	$zavoc="select uspjeh_na_polugodistu from zavrsne_ocjene where opu_id = '$opu' and predmet_id = '$_GET[predmet_id]'";
	if(!$zav=mysql_query($zavoc)){echo "<br>Nastala je greška pri izvođenju upita!<br>".mysql_query();}
	if(mysql_num_rows($zav) < 1)
	{
	$zavoc="select * from predmeti where opu_id = '$opu' and predmet_id = '$_GET[predmet_id]'";
	if(!$zav=mysql_query($zavoc)){echo "<br>Nastala je greška pri izvođenju upita!<br>".mysql_query();}
	if($red=mysql_fetch_array($zav))
		...
		$zb=0;
		$br=0;
		$prosjek="select * from ocjene where opu_id = '$opu' and predmet_id = '$_GET[predmet_id]' and datum >= '2008-09-01' and datum < '2008-12-31'";
		if(!$pros=mysql_query($prosjek)){echo "<br>Nastala je greška prilikom izvođenja upita!<br>".mysql_query();}
		while($red=mysql_fetch_array($pros))
			{
			$oc=$red["ocjena"];
			$zb+=$oc;
			$br++;
			...
			$avgoc=$zb/$br;
		echo "<br>Odaberite uspjeh na 1. polugodištu iz predmeta<br> $predmet: <br>";
		echo "<br>Posjek ocjena: ".round($avgoc,2)."<br>";
...
<br />
	...
	$unos="insert into zavrsne_ocjene(opu_id, korisnik_id, razred_id, predmet_id, uspjeh_na_polugodistu, datum) values('$opu', '$kid', '$rid', '$_GET[predmet_id]', '$_POST[ocjena]', '$datum')";
	if(!$on=mysql_query($unos)){echo "<br>Nastala je greška pri izvođenju upita!<br>".mysql_query();}
		else{header("Location: imenik.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]"); }
	}
	...
	if($red=mysql_fetch_array($zav))
		...
		$zb=0;
		$br=0;
		$prosjek="select * from ocjene where opu_id = '$opu' and predmet_id = '$_GET[predmet_id]' and datum >= '2009-01-01' and datum < '2009-07-30'";
		if(!$pros=mysql_query($prosjek)){echo "<br>Nastala je greška prilikom izvođenja upita!<br>".mysql_query();}
		while($red=mysql_fetch_array($pros))
			{
			$oc=$red["ocjena"];
			...
			$avgoc=$zb/$br;
		echo "<br>Odaberite uspjeh na kraju nastavne godine iz predmeta<br> $predmet:<br>";
		echo "<br>Posjek ocjena: ".round($avgoc,2)."<br>";
				if (!$_POST["SBunos"])
	{
		?>
...
<br />
	...
	{
	$datum= date("Y")."-".date("m")."-".date("d");
	$unos="update zavrsne_ocjene set uspjeh_na_kraju = '$_POST[ocjena]', datum = '$datum' where opu_id = '$opu' and predmet_id = '$_GET[predmet_id]'";
	if(!$on=mysql_query($unos)){echo "<br>Nastala je greška pri izvođenju upita!<br>".mysql_query();}
		else{header("Location: imenik.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]"); }
	}
		}

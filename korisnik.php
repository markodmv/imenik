...
<table width="300" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  <ul>
  <ul>
    <li><a href="dodaj_razred.php">Dodaj razred</a></li>
	</ul>
  </ul>
	<hr width="180" />
	 <ul>
...
if ($_GET["action"]=="obrisi")
	{
		...
		$rid= $_GET["razred_id"];
		$raz="SELECT        osobni_podaci_ucenika.*, razred.razred
FROM            osobni_podaci_ucenika INNER JOIN
                         Korisnik ON osobni_podaci_ucenika.korisnik_id = Korisnik.korisnik_id INNER JOIN
                         razred ON osobni_podaci_ucenika.razred_id = razred.razred_id";
			if ($razr=mysql_query($raz))
				{
					if (mysql_affected_rows() > 0)
						{
						$r= $razr["razred"];
						echo "<br>Nije moguće obrisati razred $r jer postoje učenici koji su upisani u taj razred.<br>";
						}
		else
			{
		$sql="delete from razred where korisnik_id = '$kid' and razred_id = '$rid' "; 
		if (mysql_query($sql))
			{
			if (mysql_affected_rows() > 0)
				{
				echo "<br> Razred je uspješno obrisan!<br>";
				} 
  ...
  <table width="400" border="0" align="right" cellpadding="1" cellspacing="1">
				...
				while ($redak=mysql_fetch_array($q))
					...
					<td width="100"><?=$redak["razred"]?><hr /></td>
      				<td width="150"><a href="razred.php?razred_id=<?=$redak["razred_id"]?>"> Pogledaj </a><hr /></td>
					<td width="150"><a href="?action=obrisi&razred_id=<?=$redak["razred_id"]?>"> Obriši </a><hr /></td>
					</tr>
					<?	
					}
				?>

	...
	if (mysql_num_rows($q)==0)
		...
		echo "Nema niti jednog učenika u imeniku!<br>";
				...
				while ($redak=mysql_fetch_array($q))
					...
					<tr>
					<td width="180">
					<a href="pedagoske_mjere.php?razred_id=<?=$redak["razred_id"]?>&opu_id=<?=$redak["opu_id"]?>"> <span class="style12">
					<?=$redak["rbr_ucenika"]?>
					. 
...
<table width="700" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  ?>
   ...
   <?
	$pm="select * from pedagoske_mjere where opu_id = '$opu' order by datum asc";
	if (!$p=mysql_query($pm))
	{
	...
	if (mysql_num_rows($p)==0)
		{
		echo "<br>Pedagoške mjere nisu unesene!<br>";
		} else 
				{
				while ($redak=mysql_fetch_array($p))
					{
					$b= $redak["pmjere"];
					$d= $redak["datum"];
					echo "<br>$d :<br> $b<br>";
	...
	?>
	<br />
 	<a href="pedagoske_mjere_unos.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br>
    Unesi pedagoške mjere!</a>
	</td>

...
<table width="300" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  <hr width="180">
	<ul>
  <ul>
    <li><a href="vladanje.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>">
    Povratak</a></li> 
	</ul>                                     
  </ul>
	<hr width="180">
...
<table width="700" height="506" border="0" cellpadding="0" cellspacing="0">
  ...
  if (!$_POST["SBunos"])
	...
	<?
		...
		{
		if ($_POST["tekst"] == null){header("Location: vladanje.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]");}
		else
			{
		$vlad="insert into vladanje(vladanje_pol, opu_id, razred_id, korisnik_id) values ('$_POST[tekst]', '$opu', '$rid', '$kid')";
		if(!mysql_query($vlad))
				{
				echo "<br>Došlo je do greške pri unosu!<br>". mysql_error();
					...
					{ 
					header("Location: vladanje.php?razred_id=$_GET[razred_id]&opu_id=$_GET[opu_id]");
								}
			}
		}
	?>

 ...
 <ul>
  <ul>
   <li><a href="ucenik.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>">Osobni podaci</a></li>
	...
	<ul>
	<li>
	<a href="izostanci_azuriranje.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>">Ažuriranje <br />izostanaka</a>
	</li>
	</ul>
	</ul>
	<hr width="180" />
	...
	if (mysql_num_rows($q)==0)
		...
		echo "Nema niti jednog učenika u imeniku!<br>";
				...
				while ($redak=mysql_fetch_array($q))
					...
					<tr>
					<td width="180">
					<a href="izostanci.php?razred_id=<?=$redak["razred_id"]?>&opu_id=<?=$redak["opu_id"]?>"> <span class="style14">
					<?=$redak["rbr_ucenika"]?>
					. 

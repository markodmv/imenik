...
<div class="style9 " id="Layer8">Diplomski rad: <a href="mailto:marko.vasek@gmail.com">Marko Va&scaron;ek</a>, mentor: Goran Martinović, ELEKTROTEHNIČKI FAKULTET OSIJEK </div>
</div>
...
<table width="1000" border="0" cellspacing="0" cellpadding="0" style="position:relative; left: -500px; top: 0px;">
 ...
 <br />	 
  ...
  <tr>
    <td> 
  <?
$sql="select distinct(naziv_skole) from korisnik order by naziv_skole asc";
...
?> 
  ...
  <table width="600" border="0" align="left" cellpadding="1" cellspacing="1">
				...
				while ($red=mysql_fetch_array($q))
					{
					$nz=$red["naziv_skole"];
					$skola="select * from korisnik where naziv_skole = '$nz'";
					if(!$sko=mysql_query($skola)){echo "<br>Nastala je greška prilikom izvođenja upita!<br>".mysql_query();}
					if($redak=mysql_fetch_array($sko))
						{
					?>
					<tr>
					<td width="300"><?=$redak["naziv_skole"]?><hr /></td>
					<td width="200"><?=$redak["mjesto_skole"]?><hr></td>
      				<td width="100"><a href="razredi.php?naziv_skole=<?=$redak["naziv_skole"]?>&mjesto_skole=<?=$redak["mjesto_skole"]?>"> Pogledaj </a><hr /></td>
					</tr>
					<?	
						}
					}

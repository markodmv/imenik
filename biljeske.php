	...
	if (mysql_num_rows($q)==0)
		...
		echo "Nema niti jednog učenika u imeniku!<br>";
				...
				while ($redak=mysql_fetch_array($q))
					...
					<tr>
					<td width="180">
					<a href="biljeske.php?razred_id=<?=$redak["razred_id"]?>&opu_id=<?=$redak["opu_id"]?>"> <span class="style12">
					<?=$redak["rbr_ucenika"]?>
					. 
 ...
 <br />	 
  ...
  <tr>
    ...
    <td valign="top" bgcolor="#FFFFFF">
	 <?
  echo "Bilješke :<br><br>";
  $clanak="select * from clanak58 where opu_id = '$opu'";
  if(!$clan=mysql_query($clanak)){echo "<br>Nastala je greška prilikom izvođenja upita!<br>".mysql_query();}
	...
	$bilj="select datum, biljeska from biljeske where opu_id = '$opu' order by datum asc";
	if (!$bi=mysql_query($bilj))
	{
	echo "<br>Nastala je greška pri izvođenju upita!<br />" . mysql_query();
	die();
	}
	if (mysql_num_rows($bi)==0)
		{
		echo "<br>Bilješke nisu unesene!<br>";
		} else 
				{
				while ($redak=mysql_fetch_array($bi))
					{
					$b= $redak["biljeska"];
					$d= $redak["datum"];
					if($b == null){}
	...
	?>
	<br />
 	<a href="biljeske_unos.php?razred_id=<?=$_GET["razred_id"]?>&opu_id=<?=$_GET["opu_id"]?>"><br>
    Unesi bilješke!</a>
	</td>
